...
pub fn mv(args: &[&str]) -> Result<(), String> {
    ...
    if sources.len() > 1 && !is_dir_destination {
        return Err(format!("mv: target '{}' is not a directory", destination_str));
    }
    ...
    for &source_str in sources {
        ...
        if !source.exists() {
            eprintln!("mv: cannot move '{}': No such file or directory", source_str);
            had_error = true;
            continue
        }
        ...
        if is_dir_destination {
            ...
            let file_name = match source.file_name() {
                Some(name) => name,
                None => {
                    eprintln!("mv: cannot get file name from '{}'", source_str);
                    had_error = true;
                    continue;
                }
            };
        ...
        if let Err(e) = fs::rename(source, &final_dest) {
            eprintln!("mv: cannot move '{}' to '{}': {}", source_str, final_dest.display(), e);
            had_error = true;
        }
    }
    
    if had_error {
        Err("mv: encountered one or more errors".to_string())
    } else {
        Ok(())
    }
}